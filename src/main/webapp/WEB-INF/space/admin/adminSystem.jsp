						...
						<div class="row">
							<div class="col-12 col-lg-8">
								<div class="cart-title mt-50">
									<h2>(Admin) Space Rental System</h2>

									<div style="width:auto">
										<form action="/admin/insertspacemain.controller" style="float: left">
											<input class="insert-btn btn" type="submit" value="＋新增場地資料">
										</form>
										...
										<table id="myTable" class="table table-responsive">
											<thead>
												<tr style="text-align: middle;">
													...
													<th style="flex:60%">Space</th>
													<th style="flex:60%">City</th>
													<th style="flex:60%">Add</th>
													<th style="flex:60%">People</th>
													<th style="flex:60%">Type</th>
													<th style="flex:60%">Price</th>
													<th style="flex:65%">Phone</th>
													<th style="flex:60%">Email</th>
													<th style="flex:60%">Facility</th>
													<th style="flex:100%">Introduce</th>
													<!-- 										<th style="flex:60%" >status</th> -->
													<th style="flex:100%">Picture</th>
													<th style="flex:60%"></th>
												</tr>
											</thead>
											<tbody>
												<c:forEach var="spaces" items="${spaces}">
													<tr style="font-size:small; vertical-align: middle;">
														...
														<td style="flex:60% ">${spaces.spaceName}</td>
														<td style="flex:60% ">${spaces.city}</td>
														<td style="flex:60% ">${spaces.address}</td>
														<td style="flex:60% ">${spaces.accommodate}</td>
														<td style="flex:60% ">${spaces.spaceType}</td>
														<td style="flex:60% ">${spaces.price}</td>
																	...
																	<%-- <c:when test="${spaces.status == 1}"> --%>
																		...
																		<%-- </c:when> --%>
																			<%-- </c:choose> --%>
																				...
																				<td style="flex:60% ">
																					<form
																						action="spacepreupdate.controller?spaceNo=${spaces.spaceNo}"
																						method="post">
																						<input type="submit" value="修改"
																							class="edit">
																					</form>
																					<button class="delete"
																						value="${spaces.spaceNo}">刪除</button>
																				</td>
													</tr>
												</c:forEach>
